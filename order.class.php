...
class Order extends DB_Manager {
	...
	function loadOrdersByUserId($user_id){
		...
		$query = "  SELECT order_id, cart_id, cart_total, delivery_amount, order_total, 
                    payment_method, order_datetime, order_status
					FROM tbl_orders
		...
		if (count($result) > 0) {			
		    ...
		    while ($i < count($result)) {		
                ...
                $details["delivery_amount".($i+1)] = $result[$i][3];
    ...
    function createOrder($cart_id, $user_id, $cart_total, $delivery, $order_total, $billing_name, $billing_company, $billing_email, $billing_contact, $billing_addr, $delivery_name, $delivery_company, $delivery_email, $delivery_contact, $delivery_addr, $delivery_note, $pay_method, $status) {

        $order_id = $this -> getUID('O');
        $query = "  INSERT INTO tbl_orders (order_id, cart_id, user_id, cart_total, delivery_amount, order_total, 
                    ...
                    payment_method, order_datetime, order_status) 
                    VALUES('".$order_id."','".$cart_id."','".$user_id."',".$cart_total.",".$delivery.",".$order_total.",'".
